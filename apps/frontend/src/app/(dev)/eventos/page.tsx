...
export default function PaginaEventos() {
    return (
        <div className="grid grid-cols-3 gap-5">
            {eventos.map((evento) => (
                <div key={evento.id} className="flex flex-col w-full overflow-hidden bg-zinc-500 rounded-lg">
                    <div className="relative w-full h-52">
                        <Image src={evento.imagem} fill alt={evento.nome} className="object-cover"/>
                    </div>
                    <div className="flex flex-1 flex-col p-7 gap-5 text-center items-center">
                        ...
                        <div className="flex gap-5">
                            ...
                            <Link href={`/evento/admin/${evento.alias}`} className="flex-1 botao verde">
                                Convite
                            </Link>
                        </div>
                    </div>
